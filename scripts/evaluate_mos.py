...
class CoraalDataset(Dataset):
    ...
    def validate_audio_files(self):
        for _, row in tqdm(
            self.metadata.iterrows(), total=len(self.metadata), desc="Validating audio files"):
...
def get_prediction(split, config):
    ...
    def process_batch(batch, split):
        ...
        outputs = model(wavs)
        return [
            {"loc": name[:3], "MOS": float(output), "split": split}
...
if __name__ == "__main__":
    ...
    if os.path.exists("../mos_pred.csv"):
        coraal_mos = pd.read_csv("../mos_pred.csv")
    else:
        ...
        pred_df.to_csv("../mos_pred.csv", index=False)
